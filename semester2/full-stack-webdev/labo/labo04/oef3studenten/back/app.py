...
@app.route('/api/v1/studenten', methods=['GET'])
def toon_info():
    lst_keys = list(dict_punten.keys())
    return jsonify(lst_keys)

@app.route('/api/v1/studenten/<student>', methods=['GET'])
def toon_student_info(student):
    if student in dict_punten.keys():
        studentInfo = dict_punten[student]
        print(studentInfo)
        return jsonify(detail=studentInfo, naam=student), 200
    else:
