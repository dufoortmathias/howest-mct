...
const showDestinationsForUpdate = function(jsonObject) {
    ...
    document.querySelector(".js-destinations-for-update").innerHTML = htmlstring_options;
...
const showTrain = function(jsonObject) {
	...
	document.querySelector("#vertraging").value = (jsonObject.vertraging == null ? 0 : jsonObject.vertraging)
...
const callbackAdaptTrain = function(data) {
	...
	console.log(data);
	if (data.trein_id) {
        getTrain(data.trein_id);
        window.location = "index.html";
    }
	
};
const callbackAdaptTrainError = function(data) {
    console.log("error bij aanpassen trein");
    console.log(data);
    //window.location = "index.html";
};
...
const listenToClickDestination = function() {
	...
	//treinen laden voor eerste bestemming
	currentDestinationID = buttons[0].dataset.destinationId;
    getTrainsOnDestinations(currentDestinationID);
    html_selectedCity.innerHTML = buttons[0].innerHTML;
};
...
const init = function() {
    ...
    if (html_adaptTrain) {
        //deze code wordt enkle gestart vanaf aanpassen.html
        let urlParams = new URLSearchParams(window.location.search);
        let treinid = urlParams.get("TreinID");
        if (treinid) {
    ...
    if (html_delayTrain) {
        let delayBtns = document.querySelectorAll(".js-delay-option");
        for (let btn of delayBtns) {
            btn.addEventListener("click", function() {
