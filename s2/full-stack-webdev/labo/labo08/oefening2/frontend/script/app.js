...
const showDestinationsForMap = function(jsonObject) {
    let locations = document.querySelector(".js-locations");
    for (let bestemming of jsonObject.bestemmingen){
        locations.innerHTML += `<li class="c-show-location js-location" data-lat-"${bestemming.latitude}" data-long=${bestemming.longitude}>${bestemming.stad}</li>`
    }
...
const callbackAddTrain = function(data) {
    ...
    if (data.treinid > 0) {
        ...
        html_selectedCity.innerText = document.querySelector("#add_bestemming").options[
            document.querySelector("#add_bestemming").selectedIndex
        ].innerText;
...
const listenToClickDelay = function(btn) {
    ...
    const treinid = urlParams.get("TreinID");
    const jsonobject = {
        vertraging: delay
    };
    handleData(
        `http://127.0.0.1:5000/api/v1/treinen/${treinid}/vertraging`,
        callbackUpdateDelay,
        null,
        "PUT",
        JSON.stringify(jsonobject)
...
function listenToClickShowOnMap(){
    console.log("geklikt op showOnMap");
...
const init = function() {
    ...
    if (document.querySelector(".js-delaytrain")) {
        //deze code wordt enkel gestrat vanaf vertraging.html
        console.log("add delay is actief");
        const buttons = document.querySelectorAll(".js-delay-option");
        for (let btn of buttons) {
            btn.addEventListener("click", function() {
                listenToClickDelay(this); //geef this mee als parameter aan de functie
            });
        }
    }
