...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{lsm-sparse-tree-index.png}
    \caption{LSM + Sparse tree index}
\end{figure}
...
\begin{itemize}
    ...
    \item Every delete: create new segment and merge. `Tombstone the old segment' = marking key/value pairs for deletion
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{replication-master-slave.png}
    \caption{}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{replication-multi-leader.png}
\end{figure}

\begin{itemize}
    \item More than one node can write
    \item $\Rightarrow$ write conflicts are possible
...
\begin{itemize}
    ...
    \item Do we block from writing to follower?
    \item But leader might still be alive and serving\dots
\end{itemize}

\subsection{CAP Theoreum}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item (*): last written by a write operation X older than this read operation Y
    \end{itemize}
    \item Availability: there is always a server available, if necessary: with older data
    \item PartitionTolerance: the system must keep working, even if de nodes can't communicate with each other ("network partitioning")
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{cap-theoreum.png}
...
\begin{itemize}
    \item When there are problems (partitiontolerance) with the network, we will have to choose:
    \begin{itemize}
        \item Consistency
        \item Availability
    \end{itemize}
    \item Both at the same time is NOT possible with Partition Tolerance
    \begin{itemize}
...
\begin{itemize}
    \item The system will always return data, but not always consistent (with network issues between nodes)
...
\begin{itemize}
    ...
    \item Consistency is single object! (so less transaction isolation)
    \item No CP: with many replicas, you choose for asynchronous replication
    \begin{itemize}
        \item So no strong consistency
        \item No availability (no writes)
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Is being available in 1 situation (network partition, quite rare) even that useful 
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{overview-isolation.png}
    \caption{Isolation overview}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{overview-datastores.png}
