...
\begin{itemize}
    ...
    \begin{itemize}
        \item UDP multicast - videostream, financial data
    ...
    \begin{itemize}
        ...
        \item In memory, or "spill over" on disk
        \item Low latency
    \end{itemize}
    \item (Partitioned) Log message queue (Kafka, AWS Kinesis)
    \begin{itemize}
        \item Message stays for a while
        \item Good latency (but higher)
        \item High throughput (parallel partitions)
    \end{itemize}
\end{itemize}
...
\subsection{Advantages log}


\begin{itemize}
    \item \textbf{Topic} = group of partitions that listens to the same producer
    \item Partitioning scaling
    \begin{itemize}
        \item Different disks/servers
    \end{itemize} 
...
\begin{itemize}
    ...
    \item Partition is always connected to one (or no) consumer
    \item You can loadbalance 
\end{itemize}
...
\subsection{Intro to message brokers and Kafka in more detail}

TODO: A talk is available as a video link on Leho

\subsubsection{Use case: message queue for analytics}
...
\subsection{Some use cases for message queues}
...
\begin{itemize}
    \item `Rolling analytics'
    \begin{itemize}
        \item Thumbling window (1:00-2:59, 3:00-5:59)
        \item Hopping window (1:00-2:59, 2:00-4:59)
        \item Sliding window (last 2 minutes: 1:04-3:04)
    \end{itemize}
    \item Apache Spark, Flink Kafka Streams, Azure Stream Analytics
\end{itemize}
